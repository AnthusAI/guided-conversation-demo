--[[
Support conversation flow — LLM-only guidance arm: base support flow + an orchestrator agent generates an ephemeral system suffix each turn.

The orchestrator receives only the live state summaries (no programmatic next-step hint).

Run: tactus run support_flow_llm.tac
Mock: tactus test support_flow_llm.tac --mock --param skip_hitl=true
--]]
...
-- Trace hooks (used for programmatic verification in tests).
...
You will receive a brief snapshot of the current procedure state.
You MUST output a short, actionable system message that helps the agent pick the correct next step.

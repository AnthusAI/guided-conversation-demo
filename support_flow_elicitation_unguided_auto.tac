--[[
Support conversation flow — elicitation experiment (unguided baseline, tool_choice="auto").

Appendix E reference implementation for Layer 3 ("choice-level enforcement"). Identical to
`support_flow_elicitation_unguided.tac` except `tool_choice = "auto"` — the LLM is free to
produce a free-text reply and never call a tool. Comparing the two arms isolates what
`tool_choice="required"` buys on this benchmark.

Run:  tactus run support_flow_elicitation_unguided_auto.tac
Mock: tactus test support_flow_elicitation_unguided_auto.tac --mock --param skip_hitl=true
--]]
...
guide = Agent {
    ...
    provider = "openai",
    model = GUIDE_MODEL,
    -- Layer 3 ablation: the LLM may respond without calling any tool.
    tool_choice = "auto",

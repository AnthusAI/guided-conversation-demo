--[[
Support conversation flow — programmatic guidance arm: base support flow + deterministic (state-machine) suffix.

This file is intentionally equivalent to support_flow_dynamic.tac, but renamed for 4-arm experiments.

Run: tactus run support_flow_programmatic.tac
Mock: tactus test support_flow_programmatic.tac --mock --param skip_hitl=true
--]]
...
--
-- We keep the runtime enforcement rules as-is (see tool handlers), but we also
-- compute an explicit state + next action from current structured state so the
-- programmatic arm can provide deterministic, verifiable guidance.
...
-- Same base text as support_flow_static.tac (A/B differs only by guide() suffix below).
...
local function orchestrator_suffix_for_turn()
    ...
    return "--- Orchestrator hint (ephemeral — not stored as chat history) ---\n"
        .. "State: " .. sid
        .. "\nNext suggested action: " .. nxt
        .. "\nNext token: " .. ntok
        .. "\nIssue category: "
        .. cat
        .. "\nRecording disclosure logged: "
        .. tostring(state.compliance_recording_done)
        .. "\nFee disclosure logged (billing path): "
        .. tostring(state.compliance_fee_done)
        .. "\nStill to collect: "
        .. sanitize_for_system_template(state.still_needed)
        .. "\nCollected: "
        .. sanitize_for_system_template(state.collected_summary)
end

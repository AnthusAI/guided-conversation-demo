--[[
Support conversation flow — static system prompt (disclosures, branching, approval).

Run: tactus run support_flow_static.tac
Mock: tactus test support_flow_static.tac --mock --param skip_hitl=true
--]]
...
-- Deterministic procedure trace hooks (used for programmatic verification in tests).
...
-- Shared with support_flow_dynamic.tac (identical base; dynamic appends orchestrator suffix per turn).

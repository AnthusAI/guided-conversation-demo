--[[
Support conversation flow — elicitation experiment (guided, trichotomy variant).

Appendix E reference implementation for Layer 5 ("protocol-level enforcement"). Same control
architecture as `support_flow_elicitation_guided.tac`, but the `collect_field` tool
recognizes three MCP terminal states rather than one:

  accept  — the user provided a value; validate, store, continue.
  decline — the user refused to provide this field. If the field is optional on the
            active branch, the procedure records the decline and continues. If the
            field is mandatory, the decline is rejected and the field is re-elicited.
  cancel  — the user ended the session. The procedure aborts the flow and returns
            `completed=false, cancelled=true`.

The decline/cancel signals arrive via two new user-message sentinels emitted by the
simulator's ``trichotomy`` client mode:

  [ELICITATION · DECLINE] — treat the current elicitation as declined.
  [ELICITATION · CANCEL]  — treat the current elicitation (and the whole intake) as
                            cancelled.

Run:  tactus run support_flow_elicitation_guided_trichotomy.tac
Mock: tactus test support_flow_elicitation_guided_trichotomy.tac --mock --param skip_hitl=true
--]]
...
local DECLINE_SENTINEL = "[ELICITATION · DECLINE]"
local CANCEL_SENTINEL  = "[ELICITATION · CANCEL]"

-- Fields that are safely declinable on a given branch. `decline` on any field outside
-- this allow-list is treated as an invalid terminal state and re-elicits.
-- Format: FIELD -> boolean (true means declinable).
local DECLINABLE = {
    device_model = true,   -- optional: only asked on the technical branch, can skip
    issue_summary = false, -- always required
    issue_category = false,
    account_email = false,
    callback_phone = false,
    billing_charge_acknowledged = false,
    plan_approval = false,
}
...
local function _ensure_trace_tables()
    ...
    state._protocol_events = state._protocol_events or {}
end

local function _trace_protocol(action, field, extra)
    _ensure_trace_tables()
    table.insert(state._protocol_events, {
            ...
            handler = function(args)
                ...
                if raw == nil or _trim(raw) == "" then
                    ...
                    return {
                        ...
                        instruction = "Reply to the user with the EXACT text of `message`. Then wait for the user's reply, then call collect_field again with name='" .. f .. "' and value=<the user's reply>. If the user declines to provide this field or ends the call, pass their sentinel reply (e.g. '" .. DECLINE_SENTINEL .. "' or '" .. CANCEL_SENTINEL .. "') through verbatim as the value.",
                    }
                end

                -- Trichotomy: inspect the user's reply for the MCP-style terminal sentinels
                -- before interpreting it as a field value.
                local trimmed = _trim(raw)
                if trimmed == CANCEL_SENTINEL then
                    _trace_protocol("cancel", f, "user cancelled intake")
                    _trace_step("protocol:cancel:" .. tostring(f))
                    state.cancelled = true
                    state.cancelled_field = f
                    state.last_user_message = "Okay — I've ended the call at your request. Feel free to reach back out."
                    return {
                        action = "cancel",
                        field = f,
                        message = "User cancelled the intake. Call done() with a short reason; the procedure will return completed=false, cancelled=true.",
                    }
                end
                if trimmed == DECLINE_SENTINEL then
                    if DECLINABLE[f] == true then
                        _trace_protocol("decline", f, "optional field declined")
                        _trace_step("protocol:decline:" .. tostring(f))
                        state["declined_" .. f] = true
                        -- Mark this field as "satisfied by decline" for the completion check
                        -- only when it is optional on the current branch.
                        state["form_" .. f] = "(declined)"
                        refresh_summaries()
                        local nxt = next_required_field()
                        local hint
                        if nxt == nil then
                            if all_requirements_met() then
                                hint = "All required fields are collected (the declined field was optional). Call done() with a one-line reason."
                            else
                                hint = "Optional field declined; complete any remaining disclosures or fields, then done()."
                            end
                        else
                        ...
                        return {
                            action = "declined",
                            field = f,
                            recorded = "(declined, optional)",
                            still_needed = state.still_needed,
                            next_hint = hint,
                        }
                    else
                        _trace_protocol("decline_rejected", f, "mandatory field; re-eliciting")
                        _trace_violation("collect_field:" .. tostring(f), "decline on mandatory field")
                        local msg = build_elicitation_prompt(f, "This field is required and cannot be declined.")
                        state.last_user_message = msg
                        return {
                            action = "elicit",
                            field = f,
                            error = "Field is mandatory; decline is not accepted.",
                            message = msg,
                            requested_schema = def.schema,
                            instruction = "The user tried to decline a mandatory field. Relay the message and ask again.",
                        }
                    end
                end
...
Procedure {
    ...
    output = {
        ...
        cancelled                   = field.boolean{required = false},
        cancelled_field             = field.string{required = false},
        declined_fields             = field.array{required = false},
        protocol_events             = field.array{required = false},
    },
    function(input)
        for _, k in ipairs({
            "issue_category",
        ...
        repeat
            ...
            if state.cancelled then
                print("Stopped: user cancelled (MCP protocol terminal state).")
                break
            end
        ...
        local declined_fields = {}
        for _, f in ipairs({
            "issue_summary", "issue_category", "account_email", "callback_phone",
            "device_model", "billing_charge_acknowledged", "plan_approval",
        }) do
            if state["declined_" .. f] == true then
                table.insert(declined_fields, f)
            end
        end

        local completed = done_tool.called() and all_requirements_met() and not state.cancelled
        return {
            completed = completed,
            turns = turns,
            ...
            cancelled_field = state.cancelled_field,
            declined_fields = declined_fields,
            protocol_events = state._protocol_events or {},
        }
    end,
}

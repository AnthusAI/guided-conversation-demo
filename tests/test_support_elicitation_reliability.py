...
async def _execute_once(tac_file: Path, persona_name: str, run_index: int) -> dict:
    ...
    runtime = TactusRuntime(
        procedure_id=f"support-elicitation-reliability-{tac_file.stem}-{persona_name}-r{run_index}",
...
async def test_support_elicitation_reliability(variant, tac_file, persona_name):
    ...
    out_path = REPO_ROOT / "tests" / f"results_{variant}_{persona_name}{tag}.json"
    payload = {

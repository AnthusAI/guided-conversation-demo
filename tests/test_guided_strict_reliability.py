"""
Reliability experiment: Layer 2 (decoder-level / strict JSON Schema) reference.

Runs the guided arm whose `collect_field` tool declares an explicit, closed
JSON Schema (additionalProperties=false, per-field type/enum/pattern/length
bounds) and performs a strict pre-check before the user-facing validator.
Comparison target: `support_elicitation_guided` in
`test_support_elicitation_reliability.py`.

Appendix E.2 in the paper. The arm additionally records a
`strict_violations` stream so the paper can report how often the strict-check
path fires in practice.

Usage:
  pytest tests/test_guided_strict_reliability.py -m support_guided_strict_reliability -v -s
...
STRICT_TAC = REPO_ROOT / "support_flow_elicitation_guided_strict.tac"


@pytest.mark.support_guided_strict_reliability
@pytest.mark.integration
@pytest.mark.asyncio
@pytest.mark.parametrize("client_mode", ["ideal", "non_ideal"])
@pytest.mark.parametrize("persona_name", list(SUPPORT_PERSONAS.keys()))
async def test_guided_strict_reliability(persona_name, client_mode):
    if not os.environ.get("OPENAI_API_KEY"):
        pytest.skip("Set OPENAI_API_KEY to run guided_strict reliability tests")
    if not STRICT_TAC.is_file():
        pytest.skip(f"{STRICT_TAC} not found")

    variant = "support_guided_strict"
    ...
    async def _guarded(i: int) -> tuple[int, dict]:
        async with sem:
            ...
            async def _attempt() -> dict:
                try:
                    return await _run_once(STRICT_TAC, persona_name, i, client_mode)
    ...
    for i, out in indexed:
        ...
        if out.get("success") and res.get("completed"):
            completed_count += 1
        strict_hits = len((res.get("strict_violations") or []) if isinstance(res, dict) else [])
        strict_violation_total += strict_hits
        v = None
        ...
        results.append(
            {
                ...
                "strict_violations_count": strict_hits,
    ...
    payload = {
        "experiment": "support_guided_strict",
        "variant": variant,
        ...
        "strict_violation_total": strict_violation_total,
...
def test_guided_strict_tac_exists():
    assert STRICT_TAC.is_file(), STRICT_TAC

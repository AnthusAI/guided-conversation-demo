--[[
Support conversation flow — elicitation experiment (guided, nested-elicitation variant).

Appendix D/E reference implementation for the nested elicitation protocol sketch. Same
control architecture as `support_flow_elicitation_guided.tac`, but when the procedure
needs to verify the account, `collect_field(name="account_email")` opens a nested frame
that must be satisfied (verification code) before the outer frame resumes.

Every tool return carries:

  frame_stack      — list of currently open frame descriptors, newest first. Each
                     descriptor has { depth, parent, field, state }.
  depth            — integer depth of the active frame (0 = outermost).
  parent           — name of the outer field that opened this frame, or nil.

Concretely, the outer frame here asks for `account_email`. On submit, the procedure
pushes an inner `verify_identity` frame that collects `verification_code`. Only when the
inner frame closes does the outer `account_email` frame resolve and move on to the next
field. The LLM therefore sees a visible call stack rather than a flat sequence.

Run:  tactus run support_flow_elicitation_nested.tac
Mock: tactus test support_flow_elicitation_nested.tac --mock --param skip_hitl=true
--]]
...
local VALIDATOR_ERRORS = {
    ...
    verification_code           = "Verification code must be exactly 4 digits (0-9).",
}
...
local FIELD_DEFS = {
    ...
    verification_code = {
        ...
        prompt = "We just sent a 4-digit code to your email. Please read it back to me.",
...
local function _ensure_trace_tables()
    ...
    state._frame_stack = state._frame_stack or {}
end

-- Frame stack: newest frame last. Each frame is { depth, parent, field, state }.
local function _frame_top()
    if state._frame_stack == nil or #state._frame_stack == 0 then return nil end
    return state._frame_stack[#state._frame_stack]
end

local function _frame_push(field_name, parent_name)
    _ensure_trace_tables()
    local depth = #state._frame_stack
    local frame = {
        field = field_name,
        parent = parent_name,
        depth = depth,
    ...
    table.insert(state._frame_stack, frame)
    return frame
end

local function _frame_pop(field_name)
    _ensure_trace_tables()
    if #state._frame_stack == 0 then return nil end
    local top = state._frame_stack[#state._frame_stack]
    if field_name ~= nil and top.field ~= field_name then
        return nil
    end
    table.remove(state._frame_stack, #state._frame_stack)
    top.state = "closed"
    return top
end

local function _frame_snapshot()
    local snap = {}
    for _, f in ipairs(state._frame_stack or {}) do
        table.insert(snap, { depth = f.depth, parent = f.parent, field = f.field, state = f.state })
    end
    return snap
end
...
local function all_requirements_met()
    ...
    -- Nested-protocol requirement: once account_email is collected we must also
    -- close the nested verification frame before the intake can finish.
    if (state.form_account_email or "") ~= "" and state.verification_done ~= true then
        return false
    end
    return true
end
...
local function blocked_reason(field_name)
    ...
    if not FIELD_DEFS[f] then
        return "Unknown field: " .. tostring(f) .. ". Allowed: " ..
            ...
            "billing_charge_acknowledged, plan_approval, verification_code."
    end
    -- Nested-frame gate: when an inner frame is open, only that frame's field may
    -- be collected. The LLM sees the open frame via frame_stack in every return.
    local top = _frame_top()
    if top ~= nil and top.state == "open" and top.field ~= f then
        return "A nested elicitation is open for '" .. tostring(top.field)
            .. "' (opened by '" .. tostring(top.parent or "(root)")
            .. "'). Close it first by calling collect_field(name='" .. tostring(top.field) .. "')."
    end
    ...
    inline_tools = {
        ...
        },
        {
            ...
            handler = function(args)
                ...
                -- Current frame context for the return payload.
                local current_top = _frame_top()
                local current_depth = current_top and current_top.depth or 0
                local current_parent = current_top and current_top.parent or nil
                if raw == nil or _trim(raw) == "" then
                    -- First call: push a frame if one isn't already open for this field,
                    -- then emit the elicitation prompt for the LLM to relay.
                    if current_top == nil or current_top.field ~= f then
                        local parent_field = current_top and current_top.field or nil
                        _frame_push(f, parent_field)
                        current_top = _frame_top()
                        current_depth = current_top.depth
                        current_parent = current_top.parent
                        _trace_step("frame_open:" .. tostring(f) .. ":d" .. tostring(current_depth))
                    end
                ...
                if ok then
                    -- Close this frame.
                    local popped = _frame_pop(f)
                    if popped then
                        _trace_step("frame_close:" .. tostring(f) .. ":d" .. tostring(popped.depth))
                    end

                    -- Nested-elicitation trigger: when account_email is accepted at depth 0,
                    -- open a nested frame for verification_code. The LLM sees the frame_stack
                    -- grow in its next elicit return.
                    if f == "account_email"
                        and (popped == nil or popped.depth == 0)
                        and state.verification_done ~= true
                    then
                        local inner = _frame_push("verification_code", "account_email")
                        _trace_step("frame_open:verification_code:d" .. tostring(inner.depth))
                        refresh_summaries()
                        local inner_def = FIELD_DEFS["verification_code"]
                        state.last_user_message = build_elicitation_prompt("verification_code", nil)
                        return {
                            action = "accepted",
                            recorded = f,
                            value = normalized_or_err,
                            still_needed = state.still_needed,
                            next_hint = "A nested verification frame is now open; call collect_field(name='verification_code') and relay its `message` verbatim before resuming the outer flow.",
                            nested_opened = {
                    ...
                    -- the message so the LLM (and the user) sees why. The frame stays open.
...
Procedure {
    ...
    output = {
        ...
        verification_done           = field.boolean{required = false},
        max_frame_depth             = field.number{required = false},
    },
    function(input)
        for _, k in ipairs({
            "issue_category",
        ...
        local max_depth = 0
        for _, tok in ipairs(state._step_trace or {}) do
            local d = tostring(tok):match("^frame_open:[^:]+:d(%d+)$")
            if d ~= nil then
                local n = tonumber(d) or 0
                if n > max_depth then max_depth = n end
            end
        end
        ...
        return {
            ...
            verification_done = state.verification_done == true,

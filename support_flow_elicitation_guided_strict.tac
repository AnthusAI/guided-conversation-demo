--[[
Support conversation flow — elicitation experiment (guided, strict-schema variant).

Appendix E reference implementation for Layer 2 ("decoder-level / strict JSON Schema
enforcement"). Same control architecture as `support_flow_elicitation_guided.tac`, plus:

 (1) The elicitation payload returned to the LLM carries an explicit, closed JSON Schema
     (`additionalProperties=false`, `required` list, per-field `type`/`pattern`/`enum`) and
     an explicit `strict=true` marker.
 (2) The submit path runs strict post-validation (type-tight, rejects additional keys,
     enforces length and enum bounds before the user-facing validator fires).
 (3) The system prompt instructs the LLM to produce values that will pass the schema; on
     failure, the tool re-emits the schema in the error path so the LLM sees the constraint.

NOTE on Tactus-level strict passthrough: the `Agent` DSL does not today expose a
`strict=true` flag that would force the provider to constrain the tool-call arguments via
constrained decoding or provider-side JSON-Schema enforcement (e.g. OpenAI's
`strict: true` or Anthropic's `input_schema` with strict validation). Implementing this
arm at the tool handler level is the closest approximation we can ship without framework
changes. See Appendix E.2 for the Tactus-level proposal.

Run:  tactus run support_flow_elicitation_guided_strict.tac
Mock: tactus test support_flow_elicitation_guided_strict.tac --mock --param skip_hitl=true
--]]
...
-- Elicitation-form definitions consumed by collect_field(). Each entry carries an
-- MCP-style flat JSON Schema. In the strict variant every schema is a closed object
-- (`additionalProperties=false`, single `value` property) so a schema-aware provider
-- could pin the tool-call argument shape via constrained decoding. The handler also
-- performs the equivalent checks itself on submit (Layer 2 at the tool layer) because
-- Tactus does not currently expose strict-mode passthrough at the Agent DSL level.
local function _strict_object(value_schema)
    return {
        type = "object",
...
-- Layer-2 strict-schema check run BEFORE the user-facing extractor/validator. The goal is
-- to simulate what provider-side strict JSON Schema enforcement would reject: out-of-enum
-- values, type errors, pattern mismatches, length bounds, additional properties. The
-- extractor runs after this check and only normalizes values that already pass schema.
local function _schema_check(field_name, raw_value)
    local def = FIELD_DEFS[field_name]
    if def == nil then
        return false, "unknown field"
    end
    local value_schema = (def.schema and def.schema.properties and def.schema.properties.value) or nil
    if value_schema == nil then
        return false, "missing schema for field"
    end
    if type(raw_value) ~= "string" then
        return false, "strict schema violation: value must be a JSON string"
    end
    if value_schema.minLength and #raw_value < value_schema.minLength then
        return false, "strict schema violation: value shorter than minLength=" ..
            tostring(value_schema.minLength)
    end
    if value_schema.maxLength and #raw_value > value_schema.maxLength then
        return false, "strict schema violation: value longer than maxLength=" ..
            tostring(value_schema.maxLength)
    end
    if value_schema.enum then
        local ok = false
        for _, allowed in ipairs(value_schema.enum) do
            if raw_value == allowed then ok = true; break end
        end
        if not ok then
            return false, "strict schema violation: value not in enum " ..
                table.concat(value_schema.enum, "|")
        end
    end
    if value_schema.pattern then
        -- Translate the tiny subset of ECMA regex features we actually use
        -- ("^\\d{3}-\\d{3}-\\d{4}$") into a Lua pattern so we can check here.
        if value_schema.pattern == "^\\d{3}-\\d{3}-\\d{4}$" then
            if not raw_value:match("^%d%d%d%-%d%d%d%-%d%d%d%d$") then
                return false, "strict schema violation: value does not match pattern " ..
...
local function _strict_trace_violation(field_name, reason)
    _ensure_trace_tables()
    state._strict_violations = state._strict_violations or {}
    table.insert(state._strict_violations, {
...
local function validate_and_store(field_name, raw_value)
    local f = _lower(_trim(field_name))
    if raw_value == nil or _trim(raw_value) == "" then
        _trace_violation("field:" .. tostring(f), "empty value")
        _strict_trace_violation(f, "empty value")
        return false, "value is required (the user did not provide one)"
    end
    -- Strict pre-check against the declared JSON Schema. This emulates at the tool
    -- layer what a strict-mode provider would enforce at the decoder.
    local strict_ok, strict_reason = _schema_check(f, _trim(raw_value))
    if not strict_ok then
        _trace_violation("field:" .. tostring(f), strict_reason)
        _strict_trace_violation(f, strict_reason)
        return false, strict_reason
    end
    local normalized = _extract_for_field(f, raw_value)
    if normalized == nil or normalized == "" then
        _trace_violation("field:" .. tostring(f), "empty value after extraction")
        return false, "value is required (the user did not provide one)"
    end
...
- collect_field(name [, value])
    ...
    STRICT SCHEMA: every collect_field return carries `requested_schema` (a closed JSON
    Schema object with additionalProperties=false) and `strict=true`. When you submit a
    `value`, it must satisfy `requested_schema.properties.value` exactly — type=string,
    matching enum/pattern/length bounds. Values that do not fit the schema will be rejected
    even before the user-facing validator runs.
    ...
    SUBMIT (with `value` arg): pass exactly the user's most recent reply as `value`. The
      tool validates and returns:
        ...
        {action="elicit"} with an `error`: strict-schema or domain validation failed.
          `requested_schema` shows the shape the next `value` must satisfy. Relay the new
          `message` verbatim and wait for a corrected reply.
            ...
            handler = function(args)
                ...
                if raw == nil or _trim(raw) == "" then
                    ...
                    return {
                        ...
                        strict = true,
                        instruction = "Reply to the user with the EXACT text of `message`. Then wait for the user's reply, then call collect_field again with name='" .. f .. "' and value=<the user's reply>. The `value` argument must satisfy requested_schema.properties.value exactly (type=string, enum/pattern/length bounds apply).",
                    }
                end
                ...
                if ok then
                    ...
                    -- Validation failure: re-elicit with the error and the schema baked into
                    -- the message so the LLM (and the user) sees why. In strict mode we
                    -- also echo the JSON Schema back so the LLM can correct the value shape.
                    ...
                    return {
                        ...
                        requested_schema = def.schema,
                        strict = true,
                        instruction = "The previous value was rejected against the strict JSON Schema (see requested_schema). Relay the new `message` verbatim and ask the user again; when you re-submit, the `value` argument MUST satisfy requested_schema.properties.value.",
                    }
                end
            end,
        },
...
Procedure {
    ...
    output = {
        ...
        strict_violations           = field.array{required = false},
    },
    function(input)
        for _, k in ipairs({
            "issue_category",
        ...
        return {
            ...
            strict_violations = state._strict_violations or {},
        }
    end,
}

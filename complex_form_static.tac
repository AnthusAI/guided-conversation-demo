--[[
Complex form intake — static system prompt (no {state.*} in prompt or user wrap).

Run: tactus run complex_form_static.tac
Mock: tactus test complex_form_static.tac --mock --param skip_hitl=true
--]]
...
-- Shared with complex_form_dynamic.tac (identical base instructions for static vs dynamic A/B).
...
Specification([[
Feature: Complex form intake — static prompt variant
  Agent uses a single long static system prompt with no dynamic state injection.
  Validates all fields with format checks and conditional Commercial logic.

  Scenario: Happy path Residential — static (mocked)
